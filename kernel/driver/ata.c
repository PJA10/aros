...
static return_code_t ata_read_write(int LBA, int slavebit, void *buffer, int sector_count, int write);
...
return_code_t ata_init(int slavebit) {
    ...
    if (status == 0xFF) {
        ...
        return_code = RETURN_CODE_ATA_INIT_FLOATING_BUS;
        goto cleanup;
    }
    ...
    if (status == 0) {
        ...
        return_code = RETURN_CODE_ATA_INIT_DRIVER_NOT_EXIST;
        goto cleanup;
    }
    ...
    if (inb(io_base + REG_LBA_MID) != 0 || inb(io_base + REG_LBA_HI) != 0) {
        ...
        return_code = RETURN_CODE_ATA_INIT_DRIVER_NOT_ATA;
        goto cleanup;
    }
    ...
    if (status & ERR) {
        ...
        return_code = RETURN_CODE_ATA_INIT_DRIVER_ERROR;
        goto cleanup;
    }
...
return_code_t ata_read(int sector_num, int slavebit, void *buffer, int sector_count) {
    return ata_read_write(sector_num, slavebit, buffer, sector_count, 0);
}

return_code_t ata_write(int sector_num, int slavebit, void *buffer, int sector_count) {
    return ata_read_write(sector_num, slavebit, buffer, sector_count, 1);
}
...
static return_code_t ata_read_write(int LBA, int slavebit, void *buffer, int sector_count, int write) {
    ...
    if (sector_count > 256) {
        return_code = RETURN_CODE_ATA_READ_WRITE_INVALID_SECTOR_COUNT_PARAM;
        goto cleanup;
    ...
    for (int j = 0; j < sector_count; j++) {
        ...
        if ((status & ERR) || (status & DF)) {
            ...
            return_code = RETURN_CODE_ATA_READ_WRITE_HW_ERROR;
            goto cleanup;
        }
