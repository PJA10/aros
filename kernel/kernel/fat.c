...
int fat_read(fat_file_t *file, char *buf, int size) {
    while (size) {
        ...
        //printf("file: %s at curr_cluster_num: %d, pos_in_cluster: %d size: %d\n", file->name, file->curr_cluster_num, file->pos_in_cluster, size);
        ...
        if (size < fat_boot->bytes_per_sector * fat_boot->sectors_per_cluster - file->pos_in_cluster) {
            min = size;
        }
        ...
        if (file->pos_in_cluster == 0) {
            //printf("here\n");
            file->curr_cluster_num = fat16_get_next_cluster(cluster_num);
            if (file->curr_cluster_num == -1) {
                file->eof = 1;
                break;
            }
        }
